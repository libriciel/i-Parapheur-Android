...
public abstract class LoadingTask extends AsyncTask<String, Integer, String> {
    ...
    public LoadingTask(Activity activity, DataChangeListener listener) {
        this.activity = activity; this.dataListener = listener;
    }
    ...
    @Override protected void onPreExecute() {
        if (DeviceUtils.isDebugOffline()) {
            Toast.makeText(activity, "Attention : Mode Hors Ligne.", Toast.LENGTH_SHORT).show(); showProgress();
        } else {
            ConnectivityManager connMgr = (ConnectivityManager) activity.getSystemService(Context.CONNECTIVITY_SERVICE);
            NetworkInfo networkInfo = connMgr.getActiveNetworkInfo(); if (networkInfo == null || !networkInfo.isConnected()) {
                this.cancel(true); Toast.makeText(activity, R.string.network_unreachable, Toast.LENGTH_LONG).show();
            } else {
                showProgress();
            }
        }
    ...
    @Override protected String doInBackground(String... params) {
        String error = null; try {
            ...
            error = activity.getResources().getString(e.getResId(), e.getComplement()); Sentry.capture(e);
        } return error;
    }


    @Override protected void onPostExecute(String error) {
        hideProgress(); if (error != null) {
